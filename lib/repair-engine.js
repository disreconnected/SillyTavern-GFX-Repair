...
export class TemplateRegistry {
    ...
    /**
     * Matches rendered DOM summary text, where markdown-padded spaces
     * collapse (e.g. "🗺️ Scene Plan Flow map ▸" renders as
     * "🗺️Scene PlanFlow map▸"). Compares with all whitespace removed.
     * @param {string} text Rendered summary textContent
     * @returns {boolean}
     */
    matchesDomLabel(text) {
        const squeeze = (value) => stripDecorativePrefix(cleanHeading(value))
            .replace(PLACEHOLDER_RE, '*')
            .replace(/\d+/g, '*')
            .replace(/\s+/g, '')
            .toLocaleLowerCase();
        ...
        const visit = (node) => {
            const label = squeeze(node.label);
            return (label && (candidate === label || candidate.includes(label) || label.includes(candidate)))
                || node.children.some(visit);
        };
        return this.roots.some(({ root }) => visit(root));
    }
